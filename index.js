    ...
    class VideoThumbnail {
        ...
        static {
            ...
            $body.addEventListener('click', (event) => {
                let e = event.target;
                if (!e.matches(this.selector)) return;
                event.preventDefault();
                event.stopPropagation();
        ...
        play() {
            if (this.$thumbnail === null || this.$element.classList.contains('is-active')) return;
            this.$video = document.createElement(this.tag);
            for (const [key, value] of Object.entries(this.attributes)) {
                if (value === '') this.$video.toggleAttribute(key, true);
                else this.$video.setAttribute(key, value);
            }
